...
export async function GET(req: Request) {
  try {
    ...
    // Only fetch logs for the authenticated hospital
    ...
    const grouped = logs.reduce((acc: Record<string, Record<string, number>>, log) => {
      const date = new Date(log.dispensedAt).toISOString().split('T')[0];
    ...
    console.error('[ERROR] /api/hospital-daily-usage:', error);
